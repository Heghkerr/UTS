...
public class Main {
    private Window window = new Window(1920,1080,"Proyek Grafkom");
    ...
    public void init(){
        ...
        camera.setPosition(0,0,6f);
        camera.setRotation((float)Math.toRadians(0.0f),(float)Math.toRadians(0));
        //code
        //badan
...
//BELAKANG RAMBUT
        ...
        objects.get(4).translateObject(4f,-1.5f,0f);
    }
...
//
        ...
        if (window.isKeyPressed(GLFW_KEY_LEFT_ALT)) {
            camera.addRotation(0, (float) Math.toRadians(-1 * 0.5f));
        }
...
//            objects.get(0).rotateObject((float) Math.toRadians(0.1f),-1.0f,0.0f,0.0f);
    ...
    public void loop(){
        while (window.isOpen()) {
            window.update();
            glClearColor(0f, 0.0f, 0.5f, 0.0f);
            GL.createCapabilities();
            input();
